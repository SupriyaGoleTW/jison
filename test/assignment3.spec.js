...
var treeProcessor = require('../grammar/js/assignment3');
...
describe('Tree Processor', function () {
    ...
    it('should generate a tree once it founds block of expression', function () {
        assert.equal(tree.childNode.length, 2);
        assert.equal(tree.parentNode.value, '+');
        assert.ok(tree instanceof Tree);
    });

    it('should process the tree', function () {
        assert.equal(treeProcessor.process([tree]), 30);
    });

    it('should add variable to map', function () {
        treeProcessor.addToMap({value:'x'}, val1);
        assert.equal(treeProcessor.identifierMapping['x'],val1);
    });
});